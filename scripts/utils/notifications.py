    ...
    if os.path.exists(APPRISE_CONFIG) and os.path.getsize(APPRISE_CONFIG) > 0:
        ...
        if APPRISE_MINIMUM_SECONDS_BETWEEN_NOTIFICATIONS_PER_SPECIES != "0":
            if species_last_notified.get(comName) is not None:
                try:
                    if int(timeim.time()) - species_last_notified[
                        comName
                    ] < int(
                        APPRISE_MINIMUM_SECONDS_BETWEEN_NOTIFICATIONS_PER_SPECIES
                    ):
        ...
        if (
            len(settings_dict.get("FLICKR_API_KEY")) > 0
            and "$flickrimage" in body
        ):
        ...
        if settings_dict.get("APPRISE_NOTIFY_NEW_SPECIES_EACH_DAY") == "1":
            try:
                ...
                detections = [
                    d[1]
                    for d in known_species
                    if d[0] == comName.replace("'", "")
                ]
                numberDetections = 0
                if len(detections):
                ...
                if (
                    ...
                    notify_body = (
                        ...
                        .replace("$sens", sens)
                        .replace(
                            "$flickrimage", image_url if "{" in body else ""
                        )
                        .replace("$overlap", overlap)
                        + " (first time today)"
                    )
