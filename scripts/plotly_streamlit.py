...
def time_resample(df, resample_time):
    if resample_time == "Raw":
        ...
        df_resample = (
            df.resample(resample_time)["Com_Name"]
            .aggregate("unique")
            .explode()
        )
...
if daily is False:
    ...
    if resample_time != "1D":
        ...
        if specie == "All":
            ...
            fig.add_trace(
                go.Barpolar(
                    r=detections, theta=theta, marker_color="seagreen"
                ),
                row=1,
                col=2,
            )
            ...
            with col1:
                ...
                daily = pd.crosstab(
                    df5, df5.index.date, dropna=True, margins=True
                )
                fig.add_trace(
                    go.Bar(
                        x=daily.columns[:-1],
                ...
                st.subheader(
                    ...
                    + str(
                        "{:.2f}%".format(
                            max(df2[df2["Com_Name"] == specie]["Confidence"])
                            * 100
                        )
                    )
                    + "   "
                    ...
                    + str(
                        "{:.2f}%".format(
                            np.median(
                                df2[df2["Com_Name"] == specie]["Confidence"]
                            )
                            * 100
                        )
                    )
            ...
            with col2:
                try:
                    ...
                    specie_dir = (
                        date_specie["Com_Name"].values[0].replace(" ", "_")
                    )
                    st.image(
                        userDir
            ...
            if seen:
                with colc:
                    ...
                    if verified == "False Positive":
                        ...
                        df_names = pd.concat(
                            [df_unknown, df_names], ignore_index=True
                        )
                        with cold:
        ...
        df4 = (
            df2["Com_Name"][df2["Com_Name"] == specie]
            .resample("15min")
            .count()
        )
