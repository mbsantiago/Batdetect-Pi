...
df_plt_top10_today = df_plt_today[
    df_plt_today.Com_Name.isin(plt_top10_today.index)
]
...
f, axs = plt.subplots(
    ...
    2,
    figsize=(10, 4),
    gridspec_kw=dict(width_ratios=[3, 6]),
    facecolor="#77C487",
)
plt.subplots_adjust(
...
freq_order = (
    pd.value_counts(df_plt_top10_today["Com_Name"]).iloc[:readings].index
)
...
heat = pd.crosstab(
    df_plt_top10_today["Com_Name"], df_plt_top10_today["Hour of Day"]
)
...
df_plt_Bot10_today = df_plt_today[
    df_plt_today.Com_Name.isin(plt_Bot10_today.index)
]
...
# generate y-axis order for all figures based on frequency
freq_order = (
    pd.value_counts(df_plt_Bot10_today["Com_Name"]).iloc[-readings:].index
)
...
heat = pd.crosstab(
    df_plt_Bot10_today["Com_Name"], df_plt_Bot10_today["Hour of Day"]
)
# Order heatmap Birds by frequency of occurrance
heat.index = pd.CategoricalIndex(heat.index, categories=freq_order)
heat.sort_index(level=0, inplace=True)
