...
with open(userDir + "/BirdNET-Pi/scripts/thisrun.txt", "r") as f:
    ...
    audiofmt = "." + str(
        str(
            str([i for i in this_run if i.startswith("AUDIOFMT")]).split("=")[
                1
            ]
        ).split("\\")[0]
    )
                ...
                str(
                    str(
                        [
                            i
                            for i in this_run
                            if i.startswith("PRIVACY_THRESHOLD")
                        ]
                    ).split("=")[1]
                ).split("\\")[0]
            )
    ...
    try:
        model = str(
            str(
                str([i for i in this_run if i.startswith("MODEL")]).split("=")[
                    1
                ]
            ).split("\\")[0]
        )
        sf_thresh = str(
            str(
                str([i for i in this_run if i.startswith("SF_THRESH")]).split(
                    "="
                )[1]
            ).split("\\")[0]
        )
...
def loadMetaModel():
    ...
    M_INTERPRETER = tflite.Interpreter(
        model_path=userDir
        + "/BirdNET-Pi/model/BirdNET_GLOBAL_3K_V2.2_MData_Model_FP16.tflite"
    )
                ...
                with open(userDir + "/BirdNET-Pi/BirdDB.txt", "a") as rfile:
                    for d in detections:
                        ...
                        for entry in detections[d]:
                            if entry[1] >= min_conf and (
                                (
                                    entry[0] in INCLUDE_LIST
                                    or len(INCLUDE_LIST) == 0
                                )
                                and (
                                ...
                                if not entry[0] in species_apprised_this_run:
                                    settings_dict = config_to_settings(
                                        userDir
                                        + "/BirdNET-Pi/scripts/thisrun.txt"
                                    )
                                ...
                                if birdweather_id != "99999":
                                    try:
                                        ...
                                        if (
                                            model
                                            == "BirdNET_GLOBAL_3K_V2.2_Model_FP16"
                                        ):
                                            post_algorithm = (
                                                '"algorithm": ' + '"2p2"' + ","
                                            )
